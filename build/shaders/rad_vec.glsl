...
float lineSegment(vec3 p, vec3 a, vec3 b) {
	float k = dot(p-a, b-a)/dot(b-a, b-a);
	vec3 on_line = mix(a, b, clamp(k, 0, 1));
	return distance(p, on_line);
}
...
vec2 map(vec3 p, bool[MAX_OBJECTS] hits){
	...
	for(int i = 0; i < num_segments; i++){
		if(hits[i]){
			...
			torus = torusSegment(q, angle[i], length(radius_vec[i]), TORUS_THICKNESS);
			mate = (torus < dist) ? float(i+1) : mate;
			dist = min(dist, torus);
		}
		vec3 q = vec3(p.x, p.y, -p.z) - center[i];
		vec3 rv = -radius_vec[i];
		float line = lineSegment(q, rv, 1.5*rv) - 0.075;
		mate = (line < dist) ? float(i+1) : mate;
		dist = min(dist, line);
	}
...
void main(){
	...
	if(hit){
		if(mate == 0.0){
			...
			vec3 base = hsv2rgb(vec3(0., 0.9, 0.6));
			vec3 highlight = hsv2rgb(vec3(0.02, 0.8, 0.8));
			col = mix(base, highlight, 0.5*dot(n, vec3(1))+0.5);
		}
		else if(mate > 0.0){
